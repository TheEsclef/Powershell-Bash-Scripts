#!! VARASTATUD OSA ALGAB !!
function debloat{
    Write-Host "Removing Windows 10/11 bloatware..." -ForegroundColor Cyan

    # Common Microsoft Store apps to remove
    $bloatApps = @(
        "Microsoft.3DBuilder",
        "Microsoft.XboxApp",
    ...
    # Remove provisioned apps for all users
    ...
    # --- Optional: Remove Microsoft Edge completely ---
    # This uses the uninstall command from Edge's setup folder.
    $edgePath = "C:\Program Files (x86)\Microsoft\Edge\Application"
    if (Test-Path $edgePath) {
        $edgeVersion = Get-ChildItem $edgePath | Where-Object { $_.PSIsContainer -and $_.Name -match '^[0-9]+\.' } | Select-Object -Last 1
        if ($edgeVersion) {
            $setupExe = "$($edgeVersion.FullName)\Installer\setup.exe"
